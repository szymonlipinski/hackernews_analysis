...
gg <- ggplot(data = mostActiveDomains, aes(x = date, y = count, group = domain)) +
...
The most mentioned domains are:
* github.com
* medium.com
* en.wikipedia.com
...
gg <- ggplot(data = domainsWithMostPoints, aes(x = date, y = points, group = domain)) +
  geom_line(aes(color = domain)) +
...
xkcd$html <- sprintf(
  '<span class="xkcd"><a href="%s" target="<!!>">![](%s)</a><br>%s/ </span>',
  xkcd$url,
  xkcd$embeddedUrl,
  xkcd$url
)
...
print(arstechnicaData$embeddedUrl[1:10])
```

# Github

```{r}
getDomainData(con, 'github.com')
```

# Medium

```{r}
getDomainData(con, 'medium.com')
```

# Twitter

```{r}
getDomainData(con, 'twitter.com')
```
